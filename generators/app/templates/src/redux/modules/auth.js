import app<% if(realtime) { %>, { restApp, socket }<% } %> from 'app';
...
export default function reducer(state = initialState, action = {}) {
  switch (action.type) {
    ...
    case LOGIN:<% if(oauth.length) { %>
    case OAUTHLOGIN:<% } %>
      return {
        ...state,
        loggingIn: true
      };
    case LOGIN_SUCCESS:<% if(oauth.length) { %>
    ...
    case LOGIN_FAIL:<% if(oauth.length) { %>
...
function saveAuth(response) {
  ...
  app.set('user', user); // -> set manually the user<% if(realtime) { %>
...
export function login(data) {<% if(realtime) { %>
  const socketId = socket.io.engine.id;<% } %>
  return {
    types: [LOGIN, LOGIN_SUCCESS, LOGIN_FAIL],
    promise: () => <%- realtime ? 'restApp' : 'app' %>.authenticate({
      type: 'local',
      email: data.email,
      password: data.password<% if(realtime) { %>,
      socketId<% } %>
    })
      .then(saveAuth)
      .then(setCookie)
...
export function oauthLogin(provider, data, validation = false) {<% if(realtime) { %>
  const socketId = socket.io.engine.id;<% } %>
  return {
    types: [OAUTHLOGIN, OAUTHLOGIN_SUCCESS, OAUTHLOGIN_FAIL],
    promise: () => <%- realtime ? 'restApp' : 'app' %>.service(`/auth/${provider}`)
      .create(<% if(realtime) { %>{ ...data, socketId }<% } else { %>data<% } %>)
      .then(saveAuth)
      .then(setCookie)
...
export function logout() {
  return {
    ...
    promise: () => <% if(realtime) { %>(socket.connected ? app : restApp)<% } else { %>app<% } %>.logout().then(() => {

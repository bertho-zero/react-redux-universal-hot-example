...
var webpackIsomorphicToolsPlugin = new WebpackIsomorphicToolsPlugin(require('./webpack-isomorphic-tools'));<% if(offline) { %>
...
module.exports = {
  ...
  plugins: [
    new CleanPlugin([assetsPath<%- offline ? ', \'static/service-worker.js\'' : '' %>], { root: projectRootPath }),
    ...
    webpackIsomorphicToolsPlugin<% if (offline) { %>,
    ...
    new SWPrecacheWebpackPlugin({
      ...
      runtimeCaching: [<% if(examples.indexOf('forms') === -1) { %>/* <% } %>{
        urlPattern: /\/api\/widget\/load(.*)/,
        handler: 'networkFirst',
        options: {
          debug: true
        }
      }<% if(examples.indexOf('forms') === -1) { %>*/ <% } %>]
    })
  <% } %>]
};
